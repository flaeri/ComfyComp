...
ffmpeg -hide_banner -loglevel 8 -f lavfi -i smptebars=duration=1:size=1920x1080:rate=30 -c:v hevc_nvenc -bf 2 -t 1 -f null -
if ( $LASTEXITCODE = 1) {
    write-host "HEVC B-frames not supported on your chip" -ForegroundColor Red
    $bf = 0
    Write-Host "B-Frames =" $bf
    write-host "We will continue without them :) some other features also need to be disabled" -ForegroundColor DarkYellow
} else {
