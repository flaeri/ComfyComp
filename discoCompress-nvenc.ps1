...
$audioBr = 160 #Kilobytes, audio bitrate
...
# ffmpeg encode
ffmpeg -hide_banner -i $video -c:v h264_nvenc -rc vbr -cq 26 `
-maxrate $vidBr`k -bufsize $vidBr`k -b:a $audioBr`k -pix_fmt yuv420p $dir\$baseName-$suffix`.mp4

write-host "`n"
Stop-Timer $name $startTime

$outputFile = get-childitem -Path "$dir\$baseName-$suffix`.mp4"
